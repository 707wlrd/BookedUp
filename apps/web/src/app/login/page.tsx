...
function LoginForm() {
  ...
  const isBooking = next.startsWith('/barbers/');
  ...
  const registerHref = `/register${isBooking ? `?next=${encodeURIComponent(next)}` : ''}`;

  return (
    <AuthShell
      title="Bon retour."
      subtitle={isBooking ? 'Connecte-toi pour confirmer ton rendez-vous.' : 'Connecte-toi pour continuer.'}
      footer={
        <>
          Pas encore de compte ?{' '}
          <Link href={registerHref} className="font-medium text-electric-400 hover:text-electric-300">
            Créer un compte →
          </Link>
        </>
      }
